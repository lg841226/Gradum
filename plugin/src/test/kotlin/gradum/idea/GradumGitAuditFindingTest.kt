 ...
 *
 * GradumGitAuditFindingTest.kt  2026-08-31 19:21:55 Changed by gwy
 */
...
class GradumGitAuditFindingTest {
  ...
  fun `formatMessage renders critical finding in MSVC diagnostic style`() {
    val f = finding(
      code = "S1001", level = "critical", hash = "a1b2c3d4",
      params = mapOf("add" to 500L, "dels" to 200L, "threshold" to 500L)
    )
    assertEquals(
  ...
  fun `formatMessage orders params per bundle placeholders not map order`() {
    ...
    val f = finding(code = "S2005", params = mapOf("threshold" to 500L, "avg" to 495.11538461538464))
    assertEquals(
      "alert S2005: The project averages 495 lines per commit, higher than typical LLM generation patterns.",
      f.formatMessage()
    )
  }
  ...
  fun `formatMessage rounds float params to the template precision`() {
    val f = finding(code = "S2007", params = mapOf("cv" to 0.123456789, "threshold" to 0.5))
    assertEquals(
      "alert S2007: Commit sizes are too uniform at 0.123, not matching typical developer habits.",
      f.formatMessage()
    )
  }
  ...
  fun `formatMessage rounds ratio to one decimal`() {
    val f = finding(code = "S4001", params = mapOf("ratio" to 7.844))
    assertEquals(
      "alert S4001: Deletions only 7.8%, codebase expanding, redundant code may remain.",
      f.formatMessage()
    )
  }
  ...
  fun `formatMessage handles string params verbatim`() {
    val f = finding(code = "S3003", hash = "e5f6a7b8", params = mapOf("author" to "gwy[bot]", "pattern" to "bot"))
    assertEquals(
      "alert S3003: Author \"gwy[bot]\" of commit e5f6a7b8 appears to be an automated account matching pattern \"bot\".",
      f.formatMessage()
    )
  }
  ...
  fun `formatMessage renders agent development tools for S2013`() {
    val f = finding(code = "S2013", params = mapOf("count" to 2L, "agents" to "Claude Code, OpenCode"))
    assertEquals(
      "alert S2013: 2 AI agent development tool(s) detected in use: Claude Code, OpenCode.",
      f.formatMessage()
    )
  }
  ...
  fun `formatMessage handles multiline position range`() {
    val f = finding(code = "S2001", params = mapOf("count" to 3L, "lines" to 1987L, "start_idx" to 153L, "end_idx" to 53L))
    assertEquals(
  ...
  fun `formatMessage weaves hash into S2003`() {
    val f = finding(code = "S2003", hash = "cafe1234", params = mapOf("lines" to 42L))
    assertEquals(
  ...
  fun `formatMessage weaves hash into S2006`() {
    val f = finding(code = "S2006", hash = "0badc0de", params = mapOf("n" to 5L, "ratio" to 4.0))
    assertEquals(
  ...
  fun `formatMessage weaves hash into S3001`() {
    val f = finding(code = "S3001", hash = "deadbeef", params = mapOf("lines" to 120L, "core" to 3L, "files" to 5L))
    assertEquals(
  ...
  fun `formatMessage weaves hash into S2012`() {
    val f = finding(code = "S2012", hash = "faceb00c", params = mapOf("days" to 30L, "half" to 45L))
    assertEquals(
  ...
  fun `hasRealCommitHash is true for hash-weaved codes with a real hash`() {
    assertTrue(finding(code = "S1001", hash = "a1b2c3d4", params = emptyMap()).hasRealCommitHash())
    assertTrue(finding(code = "S3003", hash = "e5f6a7b8", params = emptyMap()).hasRealCommitHash())
  }

  @Test
  fun `hasRealCommitHash is false for placeholder or non-weaved hashes`() {
    assertFalse(finding(code = "S1001", hash = "-", params = emptyMap()).hasRealCommitHash())
    assertFalse(finding(code = "S1001", hash = "", params = emptyMap()).hasRealCommitHash())
    assertFalse(finding(code = "S2001", hash = "Cluster #3", params = emptyMap()).hasRealCommitHash())
    assertFalse(finding(code = "S2002", hash = "Recent Spike", params = emptyMap()).hasRealCommitHash())
    assertFalse(finding(code = "S2005", hash = "a1b2c3d4", params = emptyMap()).hasRealCommitHash())
  }

  @Test
  fun `subject and author default to empty when absent`() {
    val f = finding(code = "S1001", hash = "a1b2c3d4", params = emptyMap())
    assertEquals(
      "",
      f.subject
    )

 ...
 *
 * GradumMarkdownTableTest.kt  2026-08-25 13:46:40 Changed by gwy
 */
...
class GradumMarkdownTableTest {
  ...
  fun `plain text without pipes passes through unchanged`() {
    ...
    assertEquals(
      MarkdownSegment.Plain(text = markdown),
      segments[0]
    )
  }
  ...
  fun `every body-row failure mode becomes an empty-rows Table`() {
    ...
    listOf(noBody, separatorButTrailingProse).forEach { markdown ->
      ...
      assertEquals(
        "expected exactly one Table in: $markdown",
        1,
        tables.size
      )
      assertEquals(
        "expected empty rows in: $markdown",
        0,
        tables[0].rows.size
      )
    }
  ...
  fun `a pathologically long line is dropped, not a single cell`() {
    val longLine: String = "x".repeat(n = 10_000)
    val markdown: String = """
      | H1 | H2 |
      | -- | -- |
      | a  | b  |
  ...
  fun `a very long table keeps all rows, no defensive cap`() {
    val rows: String = (1..600).joinToString(separator = "\n") { "| r$it |" }
    val markdown: String = """
      | H1 |
      | -- |
      $rows
  ...
  fun `a malformed line mid-table stops collection cleanly`() {
    ...
    assertTrue(plain[0].text.contains(other = "not a table row"))
  }

 ...
 *
 * GradumFootnoteTest.kt  2026-08-31 19:21:55 Changed by gwy
 */
...
class GradumFootnoteTest {
  ...
  fun `regex matches multiple commonmark footnote refs`() {
    ...
    assertEquals(
      "2",
      matches[1].groupValues[3]
    )
    assertEquals(
      "3",
      matches[2].groupValues[3]
    )
  }
  ...
  fun `regex matches multiple plain bracket numbers`() {
    ...
    assertEquals(
      "2",
      matches[1].groupValues[4]
    )
    assertEquals(
      "3",
      matches[2].groupValues[4]
    )
  }
  ...
  fun `footnote text appears in visible output`() {
    ...
    val footnoteAnnotations = render.annotated.getStringAnnotations(
      FOOTNOTE_TEXT_TAG, start = 0, end = render.annotated.length
    )
    ...
    assertEquals(
      1,
      countPlaceholders(render.annotated, base = '\uE002')
    )
    assertTrue(
      "footnote placeholder in inlineContent",
      render.inlineContent.isNotEmpty()
    )
    assertEquals(
      1,
      footnoteAnnotations.size
    )
    assertEquals(
      "note",
      footnoteAnnotations[0].item
    )
  }

  @Test
  fun `link with footnote renders both`() {
    val render = inlineRender(text = "[MDN](https://example.com)^[documentation]")
    val text = visibleText(render.annotated)
    val footnoteAnnotations = render.annotated.getStringAnnotations(
      FOOTNOTE_TEXT_TAG, start = 0, end = render.annotated.length
    )
    ...
    assertTrue(
      "Expected link text, got: $text",
      text.contains(other = "MDN")
    )
    assertEquals(
      1,
      countPlaceholders(render.annotated, base = '\uE002')
  ...
  fun `text before and after footnote preserved`() {
    val render = inlineRender(text = "hello^[note] world")
    val text = visibleText(render.annotated)
    val footnoteAnnotations = render.annotated.getStringAnnotations(
      FOOTNOTE_TEXT_TAG, start = 0, end = render.annotated.length
    )
    ...
    assertTrue(
      ...
      text.contains(other = "hello")
    )
    assertTrue(
      "Expected 'world', got: $text",
      text.contains(other = "world")
    )
    assertEquals(
      1,
      countPlaceholders(render.annotated, base = '\uE002')
  ...
  fun `multiple footnotes in same paragraph`() {
    ...
    assertTrue(
      "Expected at least 2 footnote placeholders",
      countPlaceholders(render.annotated, base = '\uE002') >= 2
    )
    assertEquals(
      2,
      footnoteAnnotations.size
    ...
    assertEquals(
      "2",
      footnoteAnnotations[1].item
    )
  }

  @Test
  fun `empty footnote skipped`() {
    val render = inlineRender(text = "text^[] more")
    val text = visibleText(render.annotated)
    assertTrue(
    ...
    assertTrue(
      "Expected 'more', got: $text",
      text.contains(other = "more")
    )
    assertFalse(
      "Should not contain ':', got: $text",
      text.contains(other = ":")
    )
  }
  ...
  fun `plain text without footnote unchanged`() {
    val render = inlineRender(text = "just plain text")
    val text = visibleText(render.annotated)
    assertEquals(
      "just plain text",
      text
  ...
  fun `footnote placeholder registered in inlineContent`() {
    ...
    assertEquals(
      "Expected 1 footnote placeholder",
      1,
  ...
  fun `footnote with CJK text renders correctly`() {
    ...
    assertEquals(
      1,
      countPlaceholders(render.annotated, '\uE002')
    )
  ...
  fun `pandoc footnote with long text`() {
    ...
    val footnoteAnnotations = render.annotated.getStringAnnotations(FOOTNOTE_TEXT_TAG, 0, render.annotated.length)
    assertEquals(
      "this is a very long footnote text that spans multiple words",
      footnoteAnnotations[0].item
    )
  }

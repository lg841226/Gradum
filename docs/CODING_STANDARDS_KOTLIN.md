...
Every Kotlin file begins with the Gradum copyright header block, followed by a blank line, then the `package`
declaration, then the import groups. The header is **required** and must not be removed:
 ...
 *
 * FileName.kt  YYYY-MM-DD HH:mm:ss Changed by gwy
 */
...
The `FileName.kt <timestamp> Changed by gwy` line is generated by the IDE template; keep whatever value the IDE stamps.
Do **not** add any other boilerplate file-level comments.
...
```kotlin
when (val result: SkillResult = skill.execute(arguments, skillContext)) {
...
```kotlin
// correct - public signature fully annotated
fun buildSchema(name: String, enabled: Boolean): Map<String, Any>

// correct - public/internal property annotated
val registeredSkills: MutableMap<String, Skill> = mutableMapOf()

// correct - local var type obvious from RHS, may omit
val stepList = buildList { /* ... */ }
val maxValueLength = 512
val tokens = command.trim().split("\\s+".toRegex())

// incorrect - public signature relies on inference
private fun readLinesInRange(lines, startLine, endLine) // = 

// incorrect - inferred local obscures the shape for later readers
val result = someLibraryCall()                          // what does someLibraryCall return?
```

When in doubt, annotate. A bare `val x = apiCall()` whose return type is not locally visible must be annotated.

Properties in `@Serializable` data classes may rely on compile-time inference (this is the only exception to the public
rule).
...
- Class must be a concrete (non-abstract, non-interface) class with a **no-argument constructor**

- Class must extend `Skill` (abstract class) in the `gradum.skill` package
...
### 3. External Plugin JARs

For external plugins, build a JAR whose class tree contains `gradum/skill/*.class`. Place the JAR on the classpath and
`SkillRegistry.findClassesInJar` picks it up automatically.
...
without any fallback / error result, is forbidden. Every caught exception must end in exactly one of:
log it, and/or propagate it as a structured failure, and/or trigger a fallback. `catch (ioException: IOException) { }`
is never acceptable.

```kotlin
// incorrect - silently swallowed
try {
    val content: String = targetFile.readText(Charsets.UTF_8)
} catch (ioException: IOException) {
    // ignore - bad
...
// incorrect - debug-only log, no fallback, no result
try {
    val content: String = targetFile.readText(Charsets.UTF_8)
} catch (ioException: IOException) {
    logger.debug("read failed")   // still swallowed from the caller's perspective
}

// correct - log + structured failure
try {
    val content: String = targetFile.readText(Charsets.UTF_8)
} catch (ioException: IOException) {
    logger.error("Failed to read $filePath", ioException)
    return makeFailure("IO_ERROR", ioException.message ?: "Read failed")
}
```
...
val errorResult: SkillResult = ...

// incorrect - one-word error names
} catch (e: IOException) { ... }
} catch (err: IOException) { ... }
val error: String = ...        // error of what?
val failure: SkillResult = ...
```

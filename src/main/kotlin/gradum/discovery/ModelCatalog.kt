 ...
 *
 * ModelCatalog.kt  2026-07-05 22:56:12 Changed by gwy
 */
        ...
        httpClient.use { client ->
            ...
            if (response.status.value == 200) {
                ...
                for ((_, providerData) in root) {
                    val modelList = providerData.jsonObject["models"]?.jsonObject ?: continue
                    for ((_, modelData) in modelList) {
                        val modelObject = modelData.jsonObject
                        val modelId = modelObject["id"]?.jsonPrimitive?.content ?: continue
                        val normalizedKey = normalizeModelName(modelId)
                ...
                logger.info("Loaded $modelCount model entries from models.dev catalog")
            } else {
...
fun lookupMetadata(modelName: String): ModelMetadata? {
    ...
    catalogCache[normalizedName]?.let { return it }
    ...
    val modelBase = normalizedName.split("-").takeWhile { part ->

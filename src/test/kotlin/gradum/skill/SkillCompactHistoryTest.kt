 ...
 *
 * SkillCompactHistoryTest.kt  2026-08-31 19:21:55 Changed by gwy
 */
...
class SkillCompactHistoryTest {
  ...
  fun `callCount increments by one per recordAndCompactHistory invocation`() {
    ...
    assertEquals(
      0,
      skill.callCount,
      "freshly created skill must start with callCount=0"
    )
    ...
    for (i in 1..4) {
      ...
      assertEquals(
        i,
        skill.callCount,
        "after $i calls, callCount should be $i"
      )
    }
    ...
    assertEquals(
      0,
      skill.callCount,
      "resetHistoryCount must zero the counter"
    )
  }
  ...
  fun `compactHistory does not over-strip when truncation has removed older own messages`() {
    ...
    assertEquals(
      "body-5",
      parsed[2]["content"],
      "body-5 must survive (within keepCount window)"
    )
    assertEquals(
      "body-6",
      parsed[3]["content"],
      "body-6 is the current call and must survive"
    )
  }
}

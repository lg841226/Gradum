 ...
 *
 * SearchSkillsEndToEndTest.kt  2026-08-31 19:21:55 Changed by gwy
 */
...
class SearchSkillsEndToEndTest {
  ...
  fun setUp() {
    ...
    write(
      relative = "gradum_kotlin.kt",
      body = """logger.info("Opening Local AI Assistant tool window")"""
    )
    write(
      relative = "src/main/kotlin/com/wangyang/localaiassistant/ui/CustomBorders.kt",
      body = "package com.wangyang.localaiassistant.ui\nclass CustomBorders\n",
    )
    write(
      relative = "src/main/kotlin/com/wangyang/localaiassistant/ui/DebugBorder.kt",
      body = "package com.wangyang.localaiassistant.ui\nclass DebugBorder\n",
    )
    write(
      relative = "src/main/kotlin/com/wangyang/localaiassistant/ui/BubbleUtils.kt",
      body = "package com.wangyang.localaiassistant.ui\nclass BubbleUtils\n// class BubbleHelper\n",
    )
    write(
      relative = "src/main/java/com/example/Legacy.java",
      body = "package com.example;\npublic class Legacy {}\n",
    )
    write(
      relative = "src/main/kotlin/com/wangyang/localaiassistant/MainToolWindow.kt",
      body = "package com.wangyang.localaiassistant\nclass MainToolWindow\n",
    )
    write(relative = "README.md", body = "# readme\n")
    write(relative = "build/classes/Foo.class/CustomBorders.class", body = "nope")
    write(relative = "bin/main/CustomBorders.kt/CustomBorders.kt", body = "package should_be_skipped\n")
    write(relative = ".gradle/cache/Foo.kt/Foo.kt", body = "package should_be_skipped\n")
    write(relative = ".idea/workspace.xml", body = "<idea-project/>")
  }
  ...
  fun `Glob finds a file by exact relative glob (regression for the original report)`() {
    ...
    val files = result.fields(key = "files") as List<*>
    assertEquals(
  ...
  fun `Glob does not match files under skipped directories`() {
    ...
    val files = result.fields(key = "files") as List<*>
    assertTrue(
      files.none { (it as String).startsWith(prefix = "bin/") },
      "bin/ is in SKIPPED_DIRECTORY_NAMES and must be walked past"
    )
    assertTrue(
      files.none { (it as String).startsWith(prefix = "build/") },
      "build/ is in SKIPPED_DIRECTORY_NAMES and must be walked past"
    )
    assertTrue(
      files.none { (it as String).startsWith(prefix = ".gradle/") },
      ".gradle/ starts with '.' and must be walked past"
    )
    assertTrue(
      files.none { (it as String).startsWith(prefix = ".idea/") },
  ...
  fun `Glob double-star matches files at any depth`() {
    ...
    val files = (result.fields(key = "files") as List<*>).map { it as String }
    assertEquals(
      setOf(
        "gradum_kotlin.kt",
    ...
    assertTrue(files.none { it.startsWith(prefix = "bin/") })
    assertTrue(files.none { it.startsWith(prefix = "build/") })
    assertTrue(files.none { it.startsWith(prefix = ".gradle/") })
  }
  ...
  fun `Glob with a path argument scopes the walk to that subdirectory`() {
    ...
    val files = (result.fields(key = "files") as List<*>).map { it as String }.toSet()
    assertEquals(
      setOf(
        "src/main/kotlin/com/wangyang/localaiassistant/ui/CustomBorders.kt",
        "src/main/kotlin/com/wangyang/localaiassistant/ui/DebugBorder.kt",
  ...
  fun `Grep include filter for kotlin sources actually finds them (regression)`() {
    ...
    assertTrue(matches.any { (it["content"] as String).contains(other = "CustomBorders") })
  }
  ...
  fun `Grep with class Border pattern and kt include finds DebugBorder and CustomBorders`() {
    ...
    val matches = (result.fields(key = "matches") as List<*>)
  ...
  fun `Grep does not search skipped directories`() {
    ...
    assertEquals(
      0,
      result.fields(key = "total_matches")
    )
  }
  ...
  fun `Grep reports which files were searched`() {
    ...
    val filesSearched = result.fields(key = "files_searched") as Int
    assertTrue(
  ...
  fun `Glob with a non-existent path returns a structured error`() {
    ...
    assertTrue(
      result.isError(),
      "Non-existent path must surface as an error, not a zero-result"
    )
  }
  ...
  fun `Glob with a malformed pattern returns a structured error`() {
    ...
    assertTrue(
      result.isError(),
      "Malformed glob must surface as an error"
    )
  }
  ...
  fun `Glob respects the limit argument`() {
    ...
    val total = result.fields(key = "total_files") as Int
    val limitApplied = result.fields(key = "limit_applied") as Boolean
    val files = result.fields(key = "files") as List<*>
    assertEquals(
      2,
      files.size
    ...
    assertTrue(
      limitApplied,
      "limit_applied must be true when result hit the cap"
    )
  }
  ...
  fun `Grep with caseSensitive true is case sensitive`() {
    ...
    assertEquals(
      0, result.fields(key = "total_matches"),
    ...
    assertTrue(
      (upperResult.fields(key = "total_matches") as Int) >= 1,
  ...
  fun `Grep with caseSensitive false is case insensitive by default`() {
    ...
    assertTrue(
      (result.fields(key = "total_matches") as Int) >= 1,
  ...
  fun `Grep limit argument is respected`() {
    ...
    assertTrue(
      limitApplied,
      "limit_applied must be true when matches hit the cap"
    )
  }
  ...
  fun `Grep with empty include matches every text file`() {
    ...
    val matches = (result.fields(key = "matches") as List<*>)
      .map { (it as Map<*, *>)["content"] as String }
    assertTrue(
      matches.any { it.contains(other = "# readme") },
  ...
  fun `Grep with brace alternation include picks up both extensions`() {
    ...
    val matches = (result.fields(key = "matches") as List<*>)
      .map { (it as Map<*, *>)["content"] as String }
    assertTrue(
      matches.any { it.contains(other = "public class Legacy") },
  ...
  fun `Grep with a file (not directory) path returns a structured error`() {
    ...
    assertTrue(
      result.isError(),
      "A file path (not directory) must be rejected, not silently walked"
    )
  }

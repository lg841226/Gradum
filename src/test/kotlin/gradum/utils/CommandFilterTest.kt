 ...
 *
 * CommandFilterTest.kt  2026-08-31 19:21:55 Changed by gwy
 */
...
class CommandFilterTest {
  ...
  fun `agent mode allows all commands that are not on the danger blocklist`() {
    listOf("ls", "cat foo.txt", "rm /tmp/junk", "git status", "echo hi").forEach { command ->
      val verdict = classifyCommand(commandText = command, ToolMode.AGENT)
      assertEquals(
        CommandVerdict.Safe,
        verdict,
  ...
  fun `read-only mode permits the read-only whitelist`() {
    listOf(
      ...
      val verdict = classifyCommand(commandText = command, ToolMode.READ_ONLY)
      assertEquals(
        CommandVerdict.Safe,
        verdict,
        "READ_ONLY should allow '$command'"
      )
    }
  }
  ...
  fun `read-only mode blocks every common write command`() {
    ...
    listOf(
      ...
      val verdict = classifyCommand(commandText = command, ToolMode.READ_ONLY)
      assertIs<CommandVerdict.Blocked>(value = verdict, message = "READ_ONLY should block '$command'")
    }
  }
  ...
  fun `read-only mode blocks shell redirect via a permissive executable name`() {
    listOf(
      ...
      val verdict = classifyCommand(commandText = command, ToolMode.READ_ONLY)
      assertIs<CommandVerdict.Blocked>(value = verdict, message = "READ_ONLY should block redirect '$command'")
    }
  }
  ...
  fun `always-on danger filter still applies in agent mode`() {
    listOf("sudo reboot", "mkfs.ext4 /dev/sdb", "rm -rf /etc").forEach { command ->
      val verdict = classifyCommand(commandText = command, ToolMode.AGENT)
      assertIs<CommandVerdict.Blocked>(value = verdict, message = "AGENT danger filter should block '$command'")
    }
  }

  @Test
  fun `default toolMode parameter is agent so existing callers stay safe`() {
    val verdict = classifyCommand(commandText = "rm junk.txt")
    assertEquals(
      CommandVerdict.Safe,
      verdict,
      "Default toolMode should be AGENT"
    )
  }
}

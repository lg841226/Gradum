...
class CompletePlanSkill(Skill):
    ...
    def execute(
        self,
        to_do_items_completed: Optional[int] = None,
        ...
        **kwargs: Any,
        ...
        if not pm.initialized:
            return make_error(
                self.name,
                "NOT_INITIALIZED",
                "No to-do item list initialized. Call to_do() first.",
            )
        ...
        if completed_count is not None:
            try:
                ...
                return make_error(
                    self.name,
                    "INVALID_PARAMETER",
                    f"completed_count must be a valid integer, got: {completed_count!r}",
                )
            ...
            if completed_count <= 0:
                return make_error(
                    self.name,
                    "INVALID_PARAMETER",
                    "completed_count must be a positive integer",
                )
            ...
            if new_total > total:
                return make_error(
                    self.name,
                    "OUT_OF_BOUNDS",
                    f"Cannot complete {completed_count} items. Only {total - pm.completed} items remaining",
                    total_tasks=total,
            ...
            tasks_with_status = [
                {
                    "index": i + 1,
                    "name": t,
                    "status": "completed" if i < pm.completed else "pending",
                }
        ...
        if to_do_items_completed is not None:
            try:
                ...
                return make_error(
                    self.name,
                    "INVALID_PARAMETER",
                    f"to_do_items_completed must be a valid integer, got: {to_do_items_completed!r}",
                )
            ...
            if to_do_items_completed > expected + 1:
                ...
                return make_error(
                    self.name,
                    "SKIP_NOT_ALLOWED",
                    f"Cannot skip {skipped} items directly. Use completed_count={skipped} to mark multiple items as done at once.",
                    total_tasks=total,
                    completed=pm.completed,
                )
            if to_do_items_completed < expected:
                return make_error(
                    self.name,
                    "CANNOT_GO_BACK",
                    f"Cannot go backwards. You already completed {expected} to-do item(s).",
                    total_tasks=total,
                    completed=pm.completed,
                )
        ...
        return make_error(
            self.name,
            "INVALID_PARAMETER",
            "Must provide either 'to_do_items_completed' (sequential mode) or 'completed_count' (batch mode)",
        )
